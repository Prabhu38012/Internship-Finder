...
export const applyForInternship = createAsyncThunk(
  ...
  async (applicationData, thunkAPI) => {
    try {
      ...
      return await applicationService.applyForInternship(
        applicationData,
        token,
      );
    } catch (error) {
    ...
    builder
      ...
      .addCase(getMyApplications.fulfilled, (state, action) => {
        ...
        state.stats = {
          ...
          pending: action.payload.data.filter((app) => app.status === "pending")
            .length,
          accepted: action.payload.data.filter(
            (app) => app.status === "accepted",
          ).length,
          rejected: action.payload.data.filter(
...
export const { reset, clearApplication, setPagination } =
