 ...
 */
const KPICard = ({ title, value, icon: Icon, iconBg = 'bg-blue-500', trend, trendUp = true }) => {
    return (
        <div className="bg-slate-800 rounded-xl p-5 shadow-md hover:shadow-lg transition-shadow duration-300">
            <div className="flex items-start justify-between">
                ...
                <div className="space-y-1">
                    ...
                    {trend && (
                        <p className={`text-sm font-medium flex items-center gap-1 ${trendUp ? 'text-green-400' : 'text-red-400'}`}>
                            <span>{trendUp ? '↑' : '↓'}</span>
                            <span>{trend}</span>
                        </p>
                    )}
