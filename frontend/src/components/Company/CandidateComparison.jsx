...
const CandidateComparison = ({
    ...
    const calculateMetrics = (candidate) => {
        ...
        const experienceScore = Math.min((profile.experience?.length || 0) * 25, 100)
        ...
        const completenessScore = Math.round(
            (skillsScore + experienceScore + educationScore + projectsScore +
                (profile.bio ? 20 : 0) + (profile.resume ? 20 : 0) +
                (profile.portfolio ? 20 : 0)) / 7 * 100 / 100
        )
    ...
    const getChartData = () => {
        const labels = ['Technical Skills', 'Experience', 'Education', 'Projects', 'Profile Complete']
                            ...
                            <Paper sx={{
                                ...
                                <Box sx={{ mb: 2 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                        <Code2 className="w-4 h-4" style={{ color: CANDIDATE_COLORS[index].border }} />
                                        <Typography variant="subtitle2" fontWeight="bold">Skills</Typography>
                                    </Box>
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                        {candidate.applicant?.studentProfile?.skills?.slice(0, 5).map((skill, i) => (
                                            <Chip
                                                key={i}
                                                label={skill}
                                                size="small"
                                        ...
                                        {(candidate.applicant?.studentProfile?.skills?.length || 0) > 5 && (
                                            <Chip
                                ...
                                <Box sx={{ mb: 2 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                        <GraduationCap className="w-4 h-4" style={{ color: CANDIDATE_COLORS[index].border }} />
                                        <Typography variant="subtitle2" fontWeight="bold">Education</Typography>
                                    </Box>
                                    {candidate.applicant?.studentProfile?.education?.slice(0, 1).map((edu, i) => (
                                        <Typography key={i} variant="body2" color="text.secondary">
                                            {edu.degree} in {edu.field} • {edu.institution}
                                        </Typography>
                                    )) || (
                                            <Typography variant="body2" color="text.secondary">
                                                {candidate.applicant?.studentProfile?.university || 'Not specified'}
                                            </Typography>
                                        )}
                                </Box>
                                ...
                                <Box sx={{ mb: 2 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                        <Briefcase className="w-4 h-4" style={{ color: CANDIDATE_COLORS[index].border }} />
                                        <Typography variant="subtitle2" fontWeight="bold">Experience</Typography>
                                    </Box>
                                    {candidate.applicant?.studentProfile?.experience?.slice(0, 1).map((exp, i) => (
                                        <Typography key={i} variant="body2" color="text.secondary">
                                ...
                                <Box sx={{ display: 'flex', gap: 1, mt: 3 }}>
                                    ...
                                    <Tooltip title="Send Email">
                                        <IconButton
                                            size="small"
                                            onClick={() => handleSendEmail(candidate.applicant?.email, candidate.applicant?.name)}
                                            sx={{
